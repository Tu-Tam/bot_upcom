...
def parse_date_range(raw_input):
    """ Tách chuỗi tham số thành danh sách ngày cần test """
    dates_to_test = []
...
def send_welcome(message):
    help_text = (
        ...
        "🔹 `/test YYYY-MM-DD` - Backtest LÔ ngày cố định\n"
        "🔹 `/test YYYY-MM-DD => DD` - Backtest LÔ theo dải ngày\n"
...
def handle_test_command(message):
    ...
    if not dates_to_test:
        bot.reply_to(message, "❌ Định dạng tham số không hợp lệ. Ví dụ: `/test 2026-08-01 => 25`", parse_mode="Markdown")
        return
        ...
        try:
            ...
            for target_date in dates_to_test:
                ...
                if hasattr(predictor, 'test_prediction_accuracy'):
                    ...
                    st_total_hits += res['song_thu_hits']
                    t5_total_hits += res['top_5_hits']
                    t10_total_hits += res['top_10_hits']

                    bt_icon = "✅" if res['bach_thu_hit'] else "❌"
                    details_list.append(
                        f"📅 **{target_date}**: BT {bt_icon} (`{res['bach_thu']}`) | "
                        f"ST: **{res['song_thu_hits']}/2** | Top5: **{res['top_5_hits']}/5** | Top10: **{res['top_10_hits']}/10**"
                    )
            ...
            bt_rate = (bt_hits / valid_days_cnt) * 100
            st_avg = st_total_hits / valid_days_cnt
            t5_avg = t5_total_hits / valid_days_cnt
            t10_avg = t10_total_hits / valid_days_cnt

            report_header = (
                f"🧪 *BÁO CÁO TEST LÔ (KHUNG 100 NGÀY)*\n"
                f"🗓 **Giai đoạn:** `{dates_to_test[0]}` ➔ `{dates_to_test[-1]}` ({valid_days_cnt} ngày)\n"
                f"------------------------------------\n"
            )
            report_footer = (
                f"\n------------------------------------\n"
                f"📊 **TỔNG KẾT TRUNG BÌNH:**\n"
                f"🔥 **Bạch Thủ Lô:** Trúng `{bt_hits}/{valid_days_cnt}` ngày (**{bt_rate:.1f}%**)\n"
                f"👯 **Song Thủ Lô:** Trung bình **{st_avg:.2f}/2** nháy/ngày\n"
                f"🌟 **Top 5 Lô đẹp:** Trung bình **{t5_avg:.2f}/5** nháy/ngày\n"
                f"📊 **Top 10 Lô đẹp:** Trung bình **{t10_avg:.2f}/10** nháy/ngày\n"
            )

            # Ghép chuỗi an toàn tránh vượt quá 4096 ký tự của Telegram
...
def handle_test_db_command(message):
    ...
    # TEST DẢI NGÀY
    if len(dates_to_test) > 1:
        msg = bot.reply_to(message, f"⏳ Đang test ĐỀ {len(dates_to_test)} ngày (Mỗi ngày lấy lùi 100 kỳ)...", parse_mode="Markdown")
