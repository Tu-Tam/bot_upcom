...
app = Flask(__name__)
@app.route('/')
def giu_song(): return "✅ Nâng cấp ưu tiên: Đã nghỉ đúng khoảng phổ biến 4-12 ngày + đang tăng tần suất gần đây!"
...
# === HÀM TÍNH ƯU TIÊN ĐANG SẮP QUAY LẠI + TĂNG GẦN ĐÂY ===
def tinh_doi_chieu_ngay(ngay_can_kiem_tra):
    ...
    for dau_so,ngay_xuat in lich_su.items():
        ...
        # ✅ TRỌNG SỐ CHÍNH: Đang nghỉ đúng khoảng phổ biến nhất được nâng điểm cao nhất
        diem_sap_quay = 45 if 4 <= so_ngay_nghi_cuoi <=12 else (20 if 13<=so_ngay_nghi_cuoi<=22 else 0)
        # Hỗ trợ mạnh: xuất hiện nhiều trong 15 ngày cuối đang có xu hướng tăng
        lan_gan = sum(1 for vt in ngay_xuat if vt >= len(du_lieu_truoc)-15)
        diem_tang_gan = lan_gan *7
        # Giảm bớt điểm chu kỳ đều đặn làm tiêu chí phân hạng phụ
        diem_deu_phu = max(0, 25 - round(do_lech_tb)*5)

        bang_diem[dau_so] = diem_sap_quay + diem_tang_gan + diem_deu_phu
...
# === LỆNH TỰ CHẠY KIỂM TRA ===
@bot.message_handler(func=lambda m: m.text and "Tự kiểm tra giai đoạn" in m.text)
def tu_chay_kiemtra(message):
    bot.send_message(message.chat.id,"🔄 Ưu tiên mạnh đuôi đang nghỉ đúng khoảng 4-12 ngày sắp quay lại + tăng tần suất gần đây...")
    tong_cong =0
    noi_chung = "📋 KẾT QUẢ ƯU TIÊN SẮP QUAY LẠI:\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    noi_chung +=f"\n📊 TRUNG BÌNH: {trung_binh}/3 đuôi trùng mỗi ngày!\n💯 Tập trung đuôi vào ngưỡng nghỉ phổ biến nhất đang đếm ngược quay lại, hỗ trợ thêm tăng gần đây!"
...
def xu_ly_du_doan(message):
    ...
    tb = tb.replace("📅 Ngày","🔮 DỰ ĐOÁN: Ưu tiên đang nghỉ đúng khoảng thường gặp nhất sắp quay lại")
